...
test('enlarged native view preserves edge input and device metrics',{timeout:45000},async()=>{
 const state=await mkdtemp('/tmp/sh-native-enlargement-');process.env.SCREENHOP_NATIVE_RUNTIME=state+'/runtime';process.env.SCREENHOP_NATIVE_PROFILES=state+'/profiles';
 const {launchNative,command}=await import('../native-launch.mjs');
 const events=[];
 const server=createServer((req,res)=>{if(req.url.startsWith('/event/')){events.push(req.url);res.end('ok');return;}res.end('<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;height:3000px}button,input{position:absolute;left:10px;width:160px;height:40px}button{top:10px}input{top:70px}</style><button onclick="fetch(\'/event/click-\'+event.isTrusted)">Click</button><button style="left:auto;right:10px" onclick="fetch(\'/event/right-\'+event.isTrusted)">Right edge</button><input oninput="fetch(\'/event/type-\'+this.value)"><script>onload=()=>fetch(\'/event/ready\');onscroll=()=>fetch(\'/event/scroll\');for(const t of [\'pointerdown\',\'mousedown\',\'touchstart\'])addEventListener(t,e=>fetch(\'/event/\'+t+\'-\'+(e.clientX||e.touches?.[0]?.clientX)+\'-\'+(e.clientY||e.touches?.[0]?.clientY)+\'-\'+e.target.tagName))</script>');});
 ...
 try{
  ...
  await exec('hyprctl',['dispatch','hl.dsp.window.float({action="set",window="address:'+current.address+'"})']);
  await exec('hyprctl',['dispatch','hl.dsp.window.resize({window="address:'+current.address+'",x=850,y=900,relative=false})']);await delay(500);
  await exec('hyprctl',['dispatch','hl.dsp.focus({window="address:'+current.address+'"})']);await delay(300);
  const g=await command(window.socket,'geometry'),info=await command(window.socket,'inspect');const scale=info.scale*g.dpr;assert.ok(scale>1,'fixture must exercise enlargement');assert.equal(info.viewport.width,375);assert.equal(info.viewport.height,667);assert.equal(info.viewport.dpr,2);
  ...
  await input(350,30,'c');assert.ok(events.includes('/event/right-true'),'right edge remains visible and clickable');
